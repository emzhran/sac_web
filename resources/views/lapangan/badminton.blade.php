@extends('layouts.app')
@section('page_title', 'Pages / Badminton')

@section('header')
<h2 class="font-semibold text-xl text-gray-800 leading-tight">
    Form Peminjaman Lapangan Badminton
</h2>
@endsection

@section('content')
<div class="bg-white p-6 rounded-lg shadow">
    <h2 class="text-xl font-bold mb-6">Form Peminjaman Lapangan Badminton</h2>
    ...
    <form id="badmintonForm" action="{{ route('badminton.store') }}" method="POST" class="space-y-4">
        @csrf
...
<script>
    const form = document.getElementById('badmintonForm');
            ...
            .then(({status, body}) => {
                if (status === 200) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Berhasil!',
                        text: body.message || 'Pemesanan lapangan badminton berhasil diajukan!',
                        confirmButtonColor: '#0ea5e9'
                    });
                    form.reset();
                } else {
