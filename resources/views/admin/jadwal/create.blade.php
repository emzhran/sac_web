...
@section('page_title', 'Tambah Jadwal Manual')
    ...
    <div class="w-full max-w-4xl">
        ...
        <div class="mb-8">
            ...
            <p class="text-sm text-gray-500">
                Admin dapat menambahkan data booking baru secara manual pada tanggal dan lapangan yang dipilih.
            </p>
        </div>
        ...
        <div class="bg-white shadow-xl shadow-indigo-500/10 rounded-2xl border border-gray-100 overflow-hidden">
            ...
            <div class="bg-indigo-600 p-6 flex items-center justify-between text-white">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-lg font-bold">Form Booking Lapangan: {{ $lapangan->nama }}</h2>
                        <p class="text-indigo-100 text-xs">Administrator</p>
                    </div>
                </div>
            </div>

            <form id="bookingForm" action="{{ route('admin.jadwal.store') }}" method="POST" class="p-8">
                @csrf
                
                <input type="hidden" name="lapangan_id" value="{{ $lapangan->id }}">
                @if ($errors->any())
                    <div class="mb-6 bg-red-50 text-red-700 p-4 rounded-xl border border-red-100 flex gap-3 items-start">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal" id="tanggal" value="{{ $tanggal }}" readonly
                                class="w-full border-gray-200 bg-gray-50 text-gray-500 rounded-xl px-4 py-3 focus:ring-0 cursor-not-allowed">
                        </div>
                    </div>

                    {{-- Right Column: Durasi Control --}}
                    <div class="bg-gray-50 rounded-2xl p-6 border border-gray-100 flex flex-col justify-center">
                        ...
                        <div class="flex items-center justify-between gap-4 mb-6">
                            ...
                            {{-- Mulai (UBAH DISINI: Jadi Readonly Input) --}}
                            <div class="flex-1">
                                <label class="text-xs text-gray-500 block mb-1 text-center">Mulai</label>
                                <div class="relative">
                                    {{-- Menggunakan $selectedJamMulai dari controller --}}
                                    <input type="time" name="jam_mulai" id="jam_mulai" value="{{ $selectedJamMulai ?? '07:00' }}" readonly
                                        class="w-full text-center border-0 bg-white shadow-sm rounded-xl py-3 text-lg font-bold text-gray-700 focus:ring-0 cursor-not-allowed">
                                </div>
                            </div>
                            ...
                            <div class="flex-1">
                                ...
                                <div class="relative">
                                    <input type="time" id="jam_selesai" name="jam_selesai" readonly
                                        class="w-full text-center border-0 bg-white shadow-sm rounded-xl py-3 text-lg font-bold text-indigo-600 focus:ring-0 cursor-not-allowed">
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    function resetJamSelesai() {
        let mulai = document.getElementById('jam_mulai').value;
        let selesaiInput = document.getElementById('jam_selesai');
        
        if (mulai) {
            let jamMulai = parseInt(mulai.split(':')[0]);
            let jamSelesai = jamMulai + 1; 
            selesaiInput.value = pad(jamSelesai) + ":00";
            updateButtonState();
        }
    }
    ...
    document.getElementById('plusBtn').onclick = function() {
        let selesaiInput = document.getElementById('jam_selesai');
        let jamSelesai = parseInt(selesaiInput.value.split(':')[0]);
        let jamMulai = parseInt(document.getElementById('jam_mulai').value.split(':')[0]);

        if (jamSelesai >= jamMulai + 3) {
             Toast.fire({ icon: 'warning', title: 'Maksimal durasi 3 jam' });
             return;
        }
        if (jamSelesai >= 23) {
            Toast.fire({ icon: 'warning', title: 'Operasional tutup jam 23:00' });
            return;
        }

        selesaiInput.value = pad(jamSelesai + 1) + ":00";
        updateButtonState();
    };
    ...
    document.getElementById('submitBtn').addEventListener('click', function() {
        if(document.getElementById('nama_pemesan_manual').value.trim() == "") {
            Toast.fire({ icon: 'error', title: 'Nama pemesan wajib diisi!' });
            return;
        }
        const nama = document.getElementById('nama_pemesan_manual').value;
        ...
        Swal.fire({
            ...
            html: `
                ...
                <p class="text-gray-600 text-sm">Pastikan data sudah benar.</p>
            `,
            icon: 'question',
