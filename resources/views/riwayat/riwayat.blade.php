...
@section('content')
    ...
    <div class="flex-1 p-4 md:p-8 bg-gray-50 min-h-screen w-full max-w-full overflow-x-hidden">
        ...
        <div
            class="bg-transparent md:bg-white md:shadow-xl md:shadow-indigo-500/5 rounded-2xl md:border border-gray-100 overflow-hidden">

            @if ($riwayats->isEmpty())
                <div
                    class="bg-white rounded-2xl p-8 flex flex-col items-center justify-center text-center shadow-sm md:shadow-none">
                    ...
                    <a href="{{ route('booking.index') }}"
                        class="px-8 py-3 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/30">
                        Cari Lapangan
                    </a>
                </div>
            @else
                        ...
                        <thead class="bg-gray-50">
                            <tr>
                                <th scope="col"
                                    ...
                                    Lapangan</th>
                                <th scope="col"
                                    class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Jadwal
                                    Main</th>
                                <th scope="col"
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors group">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div
                                                class="flex-shrink-0 h-10 w-10 rounded-lg bg-indigo-50 flex items-center justify-center text-xl">
                                            ...
                                            <div class="ml-4">
                                                <div class="text-sm font-bold text-gray-900">{{ $r->lapangan->nama ?? 'Unknown' }}
                                                </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        ...
                                        @endphp
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold {{ $statusStyles }}">
                                            ...
                                            <div class="flex items-center justify-end gap-2">
                                                @if(!$r->confirmed_at)
                                                    ...
                                                    <div class="booking-status-wrapper" data-deadline="{{ $deadlineString }}">
                                                        ...
                                                        <span
                                                            class="status-late inline-flex items-center px-3 py-1.5 bg-red-100 text-red-700 text-xs font-semibold rounded-lg border border-red-200 {{ $isAlreadyLate ? '' : 'hidden' }}">
                                                        ...
                                                        <button
                                                            onclick="checkTimeAndOpenModal('{{ route('booking.confirm', $r->id) }}', '{{ $tanggalMain }}', '{{ $jamMulai }}')"
                                                ...
                                                @else
                                                    <span
                                                        class="inline-flex items-center px-2 py-1 bg-gray-100 text-gray-600 text-xs font-medium rounded border border-gray-200">
                    ...
                    @foreach ($riwayats as $r)
                        <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 relative">
                            ...
                            <div class="flex justify-between items-start mb-3">
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center px-2 py-1 rounded-lg text-[10px] font-bold {{ $statusStyles }}">
                                    {{ $statusLabel }}
                                </span>
                            </div>
                                ...
                                <div class="flex items-center gap-2 mt-2">
                                    @if(!$r->confirmed_at)
                                        ...
                                        <div class="booking-status-wrapper w-full" data-deadline="{{ $deadlineString }}">
                                            <span
                                                class="status-late w-full inline-flex justify-center items-center px-3 py-2 bg-red-50 text-red-700 text-xs font-semibold rounded-lg border border-red-200 {{ $isAlreadyLate ? '' : 'hidden' }}">
                                                Tidak Terkonfirmasi
                                            </span>
                                    ...
                                    @else
                                        <span
                                            class="w-full inline-flex justify-center items-center px-3 py-2 bg-gray-100 text-gray-600 text-xs font-medium rounded-lg border border-gray-200">
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">Sistem akan mendeteksi lokasi GPS Anda. Fitur ini hanya
                                    dapat digunakan jika Anda berada di area <b>Universitas Muhammadiyah Yogyakarta</b>.</p>
                                <p id="locationStatus"
                                    class="mt-3 text-sm font-semibold text-gray-700 hidden bg-gray-50 p-2 rounded">Memeriksa
                                    lokasi...</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                    <form id="confirmForm" method="POST" action="" class="w-full sm:w-auto">
                        ...
                        <button type="button" id="btnCheckLocation" onclick="checkLocationAndSubmit()"
                            class="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-emerald-600 text-base font-medium text-white hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 sm:ml-3 sm:w-auto sm:text-sm">Cek
                            Lokasi & Hadir</button>
                    </form>
    ...
    <div id="errorModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-900 bg-opacity-75 transition-opacity" aria-hidden="true"
            ...
            <div
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">Maaf, Anda terdeteksi berada di luar area Universitas
                                    Muhammadiyah Yogyakarta.</p>
                                <div class="mt-4 bg-red-50 rounded-lg p-3 border border-red-100">
                                    <p class="text-xs text-red-700 font-semibold">Jarak Terdeteksi: <span id="userDistance"
                                            class="text-lg font-bold">0</span> KM</p>
                                    <p class="text-xs text-red-500 mt-1">Maksimal Jarak: 0.5 KM</p>
                                </div>
                                <p class="text-sm text-gray-500 mt-3">Silakan mendekat ke lokasi lapangan untuk melakukan
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button type="button" onclick="closeErrorModal()"
                        class="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">Mengerti</button>
                </div>
            </div>
        </div>
    </div>

    <div id="earlyAccessModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-900 bg-opacity-75 transition-opacity" aria-hidden="true"
                onclick="closeEarlyAccessModal()"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div
                class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Konfirmasi kehadiran hanya dapat dilakukan <b>minimal 1 jam</b> sebelum pemakaian
                                    lapangan.
                                </p>
